...
namespace Quick_Order
{
    public abstract class BasicPanelSettings
    {
        ...
        public const string PANELTYPE_N6000P = "N-6000P控制器";
        ...
        public static int DefaultBlackBoxCount = 1;
        public static int DefaultPrinterCount = 1;
    ...
    public class N6000PPanelSettings : BasicPanelSettings 
    {
        ...
        {
            int count = 4;
            int usedCount = ACMCount + POMCount + PrinterCount;
            if (usedCount <= 5)
            {
                count = 5 - usedCount;
            }
        ...
        {
            bool result = true;
            if (FecbusModuleCount + LPIModbusCount + NCMHighCount > 4)
            {
                result = false;
                throw new Exception("LPI、Fecbus、光纤网卡的数量加起来最多不能超过4个。");
            }
